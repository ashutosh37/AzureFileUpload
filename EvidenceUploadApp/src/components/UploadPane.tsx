...
export const UploadPane = ({
  ...
  const [isUploadFormCollapsed, setIsUploadFormCollapsed] = useState<boolean>(true);
  ...
  return (
    <div className="w-full bg-white rounded-xl shadow-2xl">
      ...
      >
        <h2 className="text-base font-bold text-gray-800">Upload Evidence</h2>
        <button
      ...
      {!isUploadFormCollapsed && (
        <div id="upload-form-content" className="p-6 pt-4">
          {accounts.length === 0 && (
          ...
          {!isContainerSelected && (
            <div className="mb-6 p-4 bg-blue-100 border-l-4 border-blue-500 text-blue-700">
              <p className="font-bold">Select a Container</p>
              <p>Please select a container and click "View/Refresh Files" before uploading.</p>
            </div>
          )}
          ...
          <div className="mb-8">
            ...
            <label 
              ...
              className={`flex flex-col items-center justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none 
                          ${accounts.length === 0 || !isContainerSelected ? 'cursor-not-allowed bg-gray-100' : 'cursor-pointer hover:border-gray-400 focus:outline-none'}
            ...
            >
                <span className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-gray-600">
                        {accounts.length > 0 && isContainerSelected ? <>Drop files to Attach, or <span className="text-blue-600 underline">browse</span></> : (accounts.length === 0 ? "Login to enable file selection" : "Select a container to enable upload")}
                    </span>
                </span>
                <input type="file" id="fileInput" multiple onChange={onFileChange} className="hidden" disabled={accounts.length === 0 || !isContainerSelected} ref={fileInputRef} />
            </label>
          ...
          <div className="mt-8">
            <button
              onClick={onUpload}
              disabled={filesToProcess.length === 0 || !isContainerSelected || uploadStatus.startsWith('Uploading') || accounts.length === 0 || !accessToken}
              className={`w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-md focus:outline-none focus:shadow-outline transition duration-150 ease-in-out ${
                (filesToProcess.length === 0 || !isContainerSelected || uploadStatus.startsWith('Uploading') || accounts.length === 0 || !accessToken) ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-lg'
              }`}
            >
